    ...
    def timesAcc(chars:List[Char], acc: Map[Char, Int]) : Map[Char,Int] = chars match {
      ...
      case x::xs => //println("testMe2 not empty List x:"+x+",xs:"+xs);
        ...
        if (acc contains(x)) {
          ...
          val newAcc: Map[Char, Int] = acc + (x -> 1)
          timesAcc(xs, newAcc)
        }
    }
    val a = timesAcc(chars, Map())
    a.toList
  }
...
/*  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
  ...
  def convert(tree: CodeTree): CodeTable = {
    def rec(subTree: CodeTree, code: List[Bit]): CodeTable = subTree match {
      case Leaf(c,_) => List((c,code))
      case Fork(leftTree, rightTree, _,_) => mergeCodeTables(rec(leftTree, code:+0), rec(rightTree, code:+1))
    }
    rec(tree, List())
  }
